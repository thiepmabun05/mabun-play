...
/* To invoke locally:
  ...
  curl -i --location --request POST 'http://127.0.0.1:54321/functions/v1/wallet-deposit' \
...
serve(async (req) => {
  ...
  const { error: txError } = await supabase
    ...
    .insert({
      ...
      type: 'deposit',
      amount,
      description: `Demo deposit via ${provider}`,
      status: 'pending',
    })
  ...
  return new Response(JSON.stringify({ success: true, message: 'Deposit initiated (demo mode)' }), {
    status: 200,
